...
if [ $? != 0 ]; then
	...
	tmux send-keys -t $SESH:editor "cd ~/Projects/superSlide/" C-m
	tmux send-keys -t $SESH:editor "nvim ." C-m

	tmux new-window -t $SESH -n "terminal"
	tmux send-keys -t $SESH:terminal "cd ~/Projects/superSlide/" C-m

	tmux new-window -t $SESH -n "console"
	tmux send-keys -t $SESH:console.1 "cd ~/Projects/superSlide" C-m
